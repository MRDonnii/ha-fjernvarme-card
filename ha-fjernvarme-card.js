...
var FjernvarmeCard = class extends HTMLElement {
	...
	_sentioRing() {
		if (!this._entityId("sentio_active")) return void 0;
		if (!this._isOn("sentio_active")) return void 0;
	...
	_sentioStatusText() {
		...
		const rawStatus = this._entityId("sentio_status") ? this._state("sentio_status") : void 0;
		const shortStatus = {
	...
	_render() {
		...
		const sentioX = 365;
		const pressureX = hasSentio ? 510 : 460;
...
var FjernvarmeCardEditor = class extends HTMLElement {
	...
	_render() {
      ...
      `;
		...
		const schemaCacheKey = `${this._language()}:0.20.1-sentio-label-fix`;
